...
import {
    ...
    CSSStyleLike,
    DomsiCssColorValue,
    DomsiCssColorValueSelector,
    DomsiCssSelector,
} from '../types/selectors/css-selector';
...
export function isCssMatch(node: HTMLNode, cssSelector: DomsiCssSelector): boolean {
    ...
    for (const [cssPropertyName, valueSelector] of Object.entries(cssSelector)) {
        if ((<any>Object).values(CssColorPropertyNames).includes(cssPropertyName)) {
            // Color property
            if (
                !isColorValueSelectorMatch(
                    elemStyles[cssPropertyName as CssPropertyNames] as DomsiCssColorValue,
                    valueSelector as DomsiCssColorValueSelector,
                )
            ) {
                return false;
            }
...
function isColorValueSelectorMatch(value: DomsiCssColorValue, valueSelector: DomsiCssColorValueSelector): boolean {
    ...
    if (valueSelector.type === 'regex') {
        const regex = (() => {
            if (typeof valueSelector.regex === 'string') {
                return new RegExp(valueSelector.regex, valueSelector.flags);
            } else if (valueSelector.regex instanceof RegExp) {
                if (typeof valueSelector.flags !== 'undefined') {
                    return new RegExp(valueSelector.regex.source, valueSelector.flags);
                } else {
                    return valueSelector.regex;
                }
            }
            throw 'Invalid regex';
        })();
    ...
    if (valueSelector.type === 'color') {
        return isColorValueMatch(value, valueSelector.color);
    }
...
export const isColorValueMatch = (() => {
    if (LOAD_RGB_CONVERTER) {
        return function isColorValueMatch(
            value1: Exclude<DomsiCssColorValue, undefined>,
            value2: Exclude<DomsiCssColorValue, undefined>,
        ): boolean {
    ...
    Object.values(CssShorthandExpanders).forEach((expander) => {
        expander.supportedProperties.forEach((propertyName) => {
            ...
            const expanded = expander.expand(
                propertyName,
                cssSelector[propertyName as keyof DomsiCssSelector] as string,
            );
...
function normalizeCssPropertyMapNames(cssPropertyMap: Record<string, string>): CSSStyleLike {
...
function cssPropertyNameToCamelCase(cssPropertyName: string): string {
    return cssPropertyName
        .toLowerCase()
        .split('-')
        .map((segment, i) => (i === 0 ? segment : segment.slice(0, 1).toUpperCase() + segment.slice(1)))
        .join('');
}
