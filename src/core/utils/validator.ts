...
export function makeConstantValidator<T>(selectorName: string, validConstants: T[], allowUndefined = false): SelectorValidator<T | undefined> {
...
export function makeArrayValidator<T>(selectorName: string, baseValidator: SelectorValidator<T>): SelectorValidator<T[]> {
    ...
    const baseValidatorName = (typeof baseValidator === 'undefined' || typeof baseValidator === 'number' || typeof baseValidator === 'string')
        ? baseValidatorType
